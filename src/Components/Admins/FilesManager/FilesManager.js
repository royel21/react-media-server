...
const FilesManager = (props) => {
  ...
  const fileFilter = useCallback(
    (flt) => {
      props.history.push(`/files/1/${getItems()}${flt ? `/${flt}` : ""}`);
    },
    [props.history]
  );
  ...
  const goToPage = (pg = 1) => {
    ...
    props.history.push(`/files/${pg}/${getItems()}${filter ? `/${filter}` : ""}`);
    return pg;
  };
  useEffect(() => {
      ...
      if (data.removed) {
        if (files.length === 1) {
          props.history.replace(
            `/files/${page > 1 ? page - 1 : page}/${getItems()}/${filter}`
          );
        } else {
          getData(page, filter, setFilesData);
        }
