...
const Slider = ({ min, max, value, onChange, previewContent, prew }) => {
  ...
  const onPreview = (e) => {
    if (prew) {
      var newPos = Math.floor(
        e.pageX - sliderRef.current.getBoundingClientRect().left
      );
      var pos = map(
        newPos - 1,
        0,
      ...
      let tempVal = Number(
        map(newPos - 1, 0, sliderRef.current.offsetWidth - 2, min, max).toFixed(
          2
        )
      );
      ...
      setpreviewData({
        pos,
        value: tempVal < 0 ? 0 : tempVal > max ? max : tempVal,
      });
    }
  };
      ...
      >
        ...
        {prew ? (
          <span
            ...
            data-title="00:00"
            style={{
              left: `calc(${previewData.pos}% - ${
                previewRef.current.offsetWidth / 2
              }px)`,
            }}
          >
